...
import { GET_ALL_TAGS } from '../../Query/getPosts';
...
const TagsTable = forwardRef(
    ({ valueSearchKeyWord, setSlugCate, setDescriptionCate, setNameCate, setEditCate, setIdCate, page, setPage, listCheckBoxCate, setListCheckBoxCate, selectedValueCate, setSelectedValueCate, loadingDelete, setLoadingDelete, setIdDm }, ref) => {
        ...
        const { loading: loadingCategoryAll, error, data: categoryAlls, refetch: refetchCategoryAll } = useQuery(GET_ALL_TAGS, {
            variables: {
                page: page,
                perPage,
        ...
        useEffect(() => {
            if (valueSearchKeyWord !== "") {
                setPage(1)
                localStorage.setItem("pagePagiTags", 1); // Lưu vào localStorage
                navigate(``)
            }
            refetchCategoryAll({
                page: page,
        ...
        const handlePageChange = (selected) => {
            ...
            localStorage.setItem("pagePagiTags", (pageCong)); // Lưu vào localStorage
            setPage(Number(localStorage.getItem("pagePagiTags"))); // Cập nhật state
            refetchCategoryAll({
                page: page,
                perPage
            }); // Gọi lại API với offset mới
            ...
            navigate(`?${localStorage.getItem('pagePagiTags') !== "" ? "&page=" + (Number(localStorage.getItem('pagePagiTags'))) : ""} `);
        };
        ...
        const handleDeleteCategories = () => {
            ...
            const deleteRequests = listCheckBoxCate.map((id) =>
                axios
                    .delete(`${baseUrl}/wp-json/wc/v3/products/tags/${id}?force=true`, {
                        headers: {
                                        ...
                                        <tbody>
                                            <tr >
                                                <td className='product_table--null' >
                                                    <p className="note-sm fw-5 cl-text mt-2">Không có thẻ nào phù hợp</p>
                                                </td>
                                            </tr>
                                        </tbody>
                                    )
